...
- `server.js` — 게임과 무관한 엔진: 방·플레이어·준비·재접속·채팅·카운트다운,
  방 공개/비공개·비밀번호 잠금, 방장이 방을 유지한 채 게임·판수 바꾸기,
  '이 판의 장면' 켜고 끄기, 그리고
...
- `score()`가 돌려주는 `banner`에 `delay`(초)와 `veil`(그동안 띄울 문구)을 붙이면,
  화면이 그만큼 배너·왕관·라운드 점수·`subDisplay`·지난 판 기록의 그 줄을 가렸다가
  한꺼번에 연다(라이어의 정체 공개). 안 붙이면 지금처럼 즉시 뜬다.
...
화면 쪽 모듈은 `moments(s, h)`로 매치가 끝났을 때 띄울 '이 판의 장면' 후보를
센 순서로 돌려줄 수 있다 — 고르는 규칙(4명 이상·문턱·하나만)은 `public/moment.js`.
또 `editor`(+`deckNoun`·`editorLabel`)로 방장이 문제를 직접 만드는
